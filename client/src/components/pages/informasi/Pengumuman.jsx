...
const Pengumuman = () => {
  ...
  useEffect(() => {
    const fetchPengumuman = async () => {
      try {
        const url = `${import.meta.env.VITE_API_URL}/informasigrup/pengumuman`;
        const response = await axios.get(url);
        setDataPengumuman(response.data.pengumuman);
      } catch (error) {
        setDataPengumuman([]);
      }
    };
  ...
  const handlePengumumanClick = async (category, slug) => {
    try {
      ...
      <Container className="py-4">
        <Row>
          <Col className="border p-4 shadow-lg bg-light rounded-3">
            <h3 className="text-center mb-4 text-primary fw-bold">Pengumuman</h3>
            {dataPengumuman.length > 0 ? (
                            ...
                            <img
                              src={
                                item.image
                                  ...
                                  : "/assets/img/berita/Pengumuman.png"
                              }
                              alt="Pengumuman"
                        ...
                        <Row className=" d-flex justify-content-between align-items-end">
                          ...
                          <Col md={8} className="d-flex justify-content-end">
                            <Button
                              ...
                              onClick={() => handlePengumumanClick(item.category, item.slug)}
                            >
                              Selengkapnya
                            </Button>
                          </Col>
              ...
              <h3 className="text-center">Belum ada Pengumuman</h3>
            )}
          </Col>
        </Row>
      </Container>
